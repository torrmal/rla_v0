window['ads_max'] = false;
Template.dashboard2.events(
  { 
    'click #dashboard_ads_box': function(){
    	if(!ads_max){
    		$('#dashboard_ads_box').removeClass('small_ads');
      		$('#dashboard_ads_box').addClass('big_ads');
      		$('#dashboard_ads_box').transition({ x: '0px', y: '0px' });
      		ads_max = true;
    	}
    	else{
    		$('#dashboard_ads_box').removeClass('big_ads');
      		$('#dashboard_ads_box').addClass('small_ads');
      		$('#dashboard_ads_box').transition({ x: '400px', y: '150px' });
      		ads_max = false;
    	}
...
Template.dashboard2.rendered = function(){
	...
	var show_more_stats = function(){
		...
		$('#dashboard_gender_markers').transition({ x: '0px', y: '-230px' });

		$('#dashboard_numbers_box').show();
		$('#dashboard_numbers_box').transition({ x: '400px', y: '-150px' });

		$('#dashboard_ads_box').show();
		$('#dashboard_ads_box').transition({ x: '400px', y: '150px' });
	...
	var update_right_pannel = function(number_of_viewers){
		...
		if(number_of_viewers == 0){
			...
			$('#dashboard_gender').hide();
			$("#faces").html('Scanning for faces ... please step back from the camera');
			update_ads(false);
	...
	var store_tick = function(gender, age){
		console.log('found');
		if(!send_data){
			send_data = {};
			send_data.tik = 0;
			send_data.timestamp = new Date().getTime();
			send_data.gender = {};
			send_data.age = {};
		}
		
		send_data.gender[gender] = send_data.gender[gender] | 0;
		send_data.gender[gender]++;
		send_data.age[age] = send_data.age[age] | 0;
		send_data.age[age]++;
		send_data.last_timestamp = new Date().getTime();
		//console.log(send_data.last_timestamp -  send_data.timestamp);
		if(send_data.last_timestamp -  send_data.timestamp >= 1000){
			//alert('aaaa');
			DeviceStats.insert(send_data);
			send_data = false;
		}
	...
	var f_listener = function(fdata, tr, tmp_canvas){
		...
		if(miss_count == 40){ 
			...
			$('#dashboard_bga2').css('visibility','inherit'); miss_count = 0; };
		...
		for(var i=0; i<fdata.data.length; i++){
			...
			var sex_preds  = determine_features(sample,'net'); 
			var age_preds = determine_features(sample,'age_net'); 
			//console.log(preds);
			if(fpreds[0].k == 1 || (fpreds[1].k == 1 &&  fpreds[1].p > .3) ){
				miss_count ++;
				continue
			};
			miss_count --;
			...
			alist[aindex] = classes_txt[sex_preds[0].k];

			aindex = aindex > 3 ? 0 : aindex +1;

			var males =  0;
			var fems = 0;
			
			for(var j in alist){
				if(alist[j] == 'Male') males++;
				else fems++;
			}

			if(fems>males) apred = 'Female';
			else if(males>fems) apred = 'Male';
			else apred = classes_txt[sex_preds[0].k];

			$('#dashboard_bga2').css('visibility','hidden');
			if (apred == 'Male'){
				$('#gender_pic').attr(
			...
			store_tick(classes_txt[sex_preds[0].k],age_classes_txt[age_preds[0].k]);
    ...
    var throttled_face = _.throttle(get_faces, 150);
        ...
        try{
        	...
        	)
        	{
          	...
          	if(!video_loaded){
          		...
          		$('#loading_message_in_circle').html('(loading camera...)');
